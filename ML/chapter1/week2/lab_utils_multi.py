...
def load_data_multi():
    data = np.loadtxt("/Users/kangmin/Desktop/personal-study/AI/ML/chapter1/data/houses.txt", delimiter=',')
    X = data[:,:2]
    y = data[:,2]
    return X, y
...
def run_gradient_descent(X,y,iterations=1000, alpha = 1e-6): # Learning rate 작게 ! 

    m,n = X.shape # 행렬 반환 

    # initialize parameters
    initial_w = np.zeros(n) # 열 개수만큼 가중치배열(벡터) 준비 
    initial_b = 0 
...
def load_house_data():
    data = np.loadtxt("/Users/kangmin/Desktop/personal-study/AI/ML/chapter1/data/houses.txt", delimiter=',', skiprows=1)
    X = data[:,:4]
    y = data[:,4]
    return X, y
