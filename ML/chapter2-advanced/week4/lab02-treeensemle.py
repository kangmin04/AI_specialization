...
# categorical data -> 문자열로 된 features이여서 one-hot encoding 해줘야함. 
cat_variables = ['Sex', 
'ChestPainType',
'RestingECG',
'ExerciseAngina',
...
# min)samples_split_list 작을수록 accuracy가 1에 수렴함 -> overfitting! 
# min_samples를 증가시키면 overfitting이 해결되며, test_val에서도 나름 잘 작동한다. 
accuracy_list_train = []
accuracy_list_val = []
...
# 위의 두 결과를 토대로 
# max_depth = 4
# min_samples_split = 50 이 best fit이라 결론낼수이싿. 
...
n_estimators_list = [10,50,100,500] # n_estimators which is the number of Decision Trees that make up the Random Forest.
